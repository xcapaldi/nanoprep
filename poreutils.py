...
def estimate_diameter(solution_conductivity = 11.53,
    ...
    if channel_conductance > 0:
        ...
        pore_conductance = (((conductance ** -1)
                            - (branch_conductance ** -1))
                            ** -1)
    
        # need to calculate error on the isolated pore conductance
        error_pore = (pore_conductance
                        * (math.sqrt((((error_conductance)
                                        /(conductance ** 2)) ** 2)
                                        + (((error_branch)
                                        /(branch_conductance ** 2)) ** 2)))
                        / ((error_conductance ** -1) - (error_branch ** -1)))
    else:
        pore_conductance = conductance
        error_pore = error_conductance
        
    K = math.sqrt(1 + ((16 * solution_conductivity * effective_length)
                       /(math.pi * pore_conductance)))
    
    diameter = (pore_conductance / (2 * solution_conductivity)) * (1 + K)
    
    error_diameter = math.sqrt(((((1 + K)
                                  / (2 * solution_conductivity))
                                 - ((4 * effective_length)
                                    / (math.pi * pore_conductance * K)))
                                ** 2)
                               * error_pore ** 2
                               + ((((4 * effective_length)
                                     / (math.pi * solution_conductivity * K))
                                   - ((pore_conductance
                                      / (2 * solution_conductivity ** 2))
                                      * (1 + K)))
                                  ** 2)
...
def estimate_length(solution_conductivity = 11.53,
                    dna_diameter = 2.2E-9,
                    delta_conductance = 0):
    """
